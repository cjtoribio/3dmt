...
#include <vector>
using namespace std;
#define for_each(it,F) for(typeof(F.begin()) it = F.begin(); it != F.end(); ++it)
struct SignalFilter
{
	int FRONT_FRAMES = 0;
	int MIDDLE_FRAMES = 128;
	int BACK_FRAMES = 0;
	...
	{
		F.push_back(magnitude);
		if(F.size() > FRONT_FRAMES + MIDDLE_FRAMES + BACK_FRAMES)
			F.pop_front();
	}
	static void doNothing(CD *CV, const int SZ){}
	vector<double> getProcessedSignal(void (*process)(CD *CV,const int SZ) = doNothing)
	{
		vector<double> V(F.begin(),F.end());
